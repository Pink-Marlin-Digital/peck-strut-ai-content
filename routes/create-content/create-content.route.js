...
export function registerCreateContentRoute(server) {
  console.info('[registerCreateContentRoute] Registering /create-content route');
  ...
  fs.watchFile(TEMPLATE_PATH, () => {
    console.info('[registerCreateContentRoute] Detected change in prompt-template.md, reloading template');
    compiledTemplate = loadTemplate();
  });

  server.post("/create-content", async (request, reply) => {
    const { prompt, persona, sentiment } = request.body;
    console.info('[POST /create-content] Received request', { prompt, persona, sentiment });
    if (!prompt || !persona || !sentiment) {
      console.warn('[POST /create-content] Missing required fields', { prompt, persona, sentiment });
      return reply
        .code(400)
        .send({ error: "Missing required fields: prompt, persona, sentiment" });
    }
    ...
    if (!apiKey) {
      console.warn('[POST /create-content] OPENAI_API_KEY not set in environment');
      return reply.code(500).send({ error: "OPENAI_API_KEY not set in environment." });
    }
    try {
      console.info('[POST /create-content] Sending prompt to OpenAI', { model, baseURL });
      ...
      console.info('[POST /create-content] Sending second LLM call for hashtags');
      ...
      console.info('[POST /create-content] OpenAI hashtag response metadata', hashtagMeta);
      let hashtags = [];
      try {
        hashtags = JSON.parse(hashtagCompletion.choices?.[0]?.message?.content || '[]');
      } catch (e) {
        console.warn('[POST /create-content] Failed to parse hashtags JSON', { error: e.message });
      }
      return reply.send({ message, hashtags });
    } catch (err) {
      console.warn('[POST /create-content] Error contacting OpenAI', { error: err.message });
      return reply.code(500).send({ error: "Failed to contact OpenAI", details: err.message });
    }
  });
}
